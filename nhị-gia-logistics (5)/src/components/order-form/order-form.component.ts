...
import { DataService, Order, Attachment, DEPARTMENTS, SENDERS, Sender } from '../../services/data.service';
  ...
  template: `
    <div class="bg-white rounded-lg shadow-lg p-6 max-h-[90vh] overflow-y-auto">
      <div class="flex justify-between items-center mb-6 border-b pb-4">
        ...
        <button (click)="cancel.emit()" class="text-gray-500 hover:text-gray-700">
      ...
      @if (form) {
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-4">
              ...
              <div class="flex">
                ...
                <div class="ml-3">
                  <p class="text-sm text-yellow-700">
                    <strong>Yêu cầu bổ sung:</strong> {{ orderData()?.requestNote }}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="bg-blue-50 p-4 rounded border border-blue-100 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1">Bộ Phận</label>
            ...
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1">Người Giao</label>
              <div class="flex flex-col">
                <select formControlName="senderName" (change)="onSenderChange()"
                  class="w-full border rounded-md py-2 px-3 focus:ring-blue-500 focus:border-blue-500 bg-white shadow-sm">
                ...
                @if (selectedSenderPhone()) {
                  <a [href]="'tel:' + selectedSenderPhone()" class="text-xs text-blue-600 mt-1 font-medium flex items-center hover:underline">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <!-- Phone & Auto-fill -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Số điện thoại khách</label>
              <div class="relative">
                <input type="text" formControlName="phone" (blur)="onPhoneBlur()"
            ...
            <!-- Company -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Tên công ty / Khách hàng</label>
          ...
          <div class="bg-gray-50 p-4 rounded border border-gray-100 space-y-4">
            <h4 class="text-xs font-bold text-gray-400 uppercase tracking-widest">Địa chỉ chi tiết</h4>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Số địa chỉ (Tên đường, số nhà)</label>
                <input type="text" formControlName="addressLine" placeholder="Vd: 186-188 Nguyễn Duy"
                  class="w-full border rounded-md py-2 px-3 focus:ring-blue-500 focus:border-blue-500">
              </div>
              <div>
                 <label class="block text-sm font-medium text-gray-700 mb-1">Tỉnh/TP</label>
                 <select formControlName="province" (change)="onProvinceChange($event)" class="w-full border rounded-md py-2 px-3 bg-white">
              ...
              <div>
                 ...
                 <select formControlName="district" (change)="onDistrictChange($event)" class="w-full border rounded-md py-2 px-3 bg-white">
                    <option value="">-- Chọn Quận/Huyện --</option>
                    <option *ngFor="let d of districts[form.get('province')?.value] || []" [value]="d">{{ d }}</option>
                 </select>
              </div>
              <div>
                 <label class="block text-sm font-medium text-gray-700 mb-1">Phường/Xã</label>
                 <input type="text" formControlName="ward" placeholder="Vd: Phường 9"
                    class="w-full border rounded-md py-2 px-3 focus:ring-blue-500 focus:border-blue-500">
              </div>
            </div>
            <div class="text-[10px] text-gray-500 italic bg-white p-2 border rounded">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Người liên hệ (Gặp)</label>
          ...
          <div>
            ...
            <input type="text" formControlName="purpose"
              class="w-full border rounded-md py-2 px-3 focus:ring-blue-500 focus:border-blue-500" placeholder="Vd: Giao hồ sơ, Lấy dấu...">
          </div>
          
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Ghi chú</label>
            <textarea formControlName="notes" rows="2"
              class="w-full border rounded-md py-2 px-3 focus:ring-blue-500 focus:border-blue-500"></textarea>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded">
            <div>
              ...
              <input type="text" formControlName="amountVND"
                (input)="formatCurrency($event, 'amountVND')"
                class="w-full border rounded-md py-2 px-3 text-right font-mono" placeholder="0">
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Số tiền (USD)</label>
               <input type="text" formControlName="amountUSD"
          ...
          <div class="bg-white border rounded-lg overflow-hidden">
            <div class="bg-gray-100 px-4 py-2 border-b flex justify-between items-center">
              <label class="text-sm font-bold text-gray-700 uppercase">Hồ sơ đính kèm</label>
              <button type="button" (click)="addNewAttachment()" class="bg-blue-600 text-white w-6 h-6 rounded-full flex items-center justify-center shadow-sm hover:bg-blue-700 font-bold">+</button>
            </div>
            ...
            <div class="p-4 space-y-3">
              <div *ngFor="let att of customAttachments; let i = index" class="flex items-center gap-3 animate-fade-in group">
                 <input type="checkbox" [checked]="att.checked" (change)="toggleCustomAtt(i)" class="w-4 h-4 text-blue-600 rounded">
                 <div class="flex-1">
                    <input type="text" [(ngModel)]="att.name" [ngModelOptions]="{standalone: true}" placeholder="Tên hồ sơ..."
                 ...
                 <div class="flex items-center border rounded h-8">
                    <button type="button" (click)="updateCustomQty(i, -1)" class="px-2 py-1 bg-gray-50 hover:bg-gray-200">-</button>
                    <span class="px-3 text-xs font-mono font-bold">{{ att.qty }}</span>
                    <button type="button" (click)="updateCustomQty(i, 1)" class="px-2 py-1 bg-gray-50 hover:bg-gray-200">+</button>
                 </div>
                 <button type="button" (click)="removeCustomAtt(i)" class="text-red-400 hover:text-red-600 font-bold px-2">X</button>
              </div>
              <div *ngIf="customAttachments.length === 0" class="text-center py-4 text-gray-400 text-xs italic border-2 border-dashed rounded">Bấm "+" để thêm hồ sơ cần bàn giao.</div>
            </div>
          </div>
          ...
          <div class="bg-white border rounded-lg overflow-hidden">
            <div class="bg-gray-100 px-4 py-2 border-b flex justify-between items-center">
               <label class="text-sm font-bold text-gray-700 uppercase">Tài liệu bổ sung (PDF, Ảnh, Word)</label>
            </div>
            <div class="p-4">
               <div class="relative border-2 border-dashed border-gray-200 rounded-lg p-6 flex flex-col items-center justify-center hover:bg-blue-50 transition-colors cursor-pointer group">
                  <input type="file" multiple (change)="onFilesUploaded($event)" class="absolute inset-0 opacity-0 cursor-pointer">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-gray-400 mb-2 group-hover:text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                  </svg>
                  <p class="text-xs text-gray-500">Chọn hoặc kéo thả tệp vào đây</p>
                  <p class="text-[10px] text-gray-400 mt-1 uppercase font-bold">Hỗ trợ File PDF, Hình ảnh, Word (.doc, .docx)</p>
               </div>

               @if (uploadedFileList().length > 0) {
                 <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
                    @for (file of uploadedFileList(); track $index) {
                      <div class="flex items-center justify-between p-2 bg-gray-50 rounded border text-xs border-gray-100 group">
                         <div class="flex items-center gap-2 truncate">
                            <span class="text-blue-500">📎</span>
                            <span class="truncate font-medium">{{ file.name }}</span>
                            <span class="text-[8px] text-gray-400 uppercase">({{ file.type }})</span>
                         </div>
                         <button type="button" (click)="removeUploadedFile($index)" class="text-gray-400 hover:text-red-500">✕</button>
                      </div>
                    }
                 </div>
               }
          ...
          <!-- Actions -->
          <div class="flex items-center justify-end space-x-4 pt-4 border-t mt-4">
            <button type="button" (click)="cancel.emit()" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">
              Hủy Bỏ
            </button>
            <button type="submit" [disabled]="form.invalid" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-black text-white bg-blue-600 hover:bg-blue-700 focus:outline-none disabled:opacity-50 uppercase tracking-tight">
...
export class OrderFormComponent implements OnInit {
  ...
  onPhoneBlur() {
    ...
    if (phone) {
      const customer = this.dataService.findCustomerByPhone(phone);
      if (customer) {
  ...
  onCompanyChange() {
    ...
    if (company) {
      const customer = this.dataService.findCustomerByCompany(company);
      if (customer) {
        this.fillCustomerData(customer);
      }
    }
  ...
  toggleCustomAtt(index: number) {
    this.customAttachments[index].checked = !this.customAttachments[index].checked;
  }
  ...
  async onFilesUploaded(event: any) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      reader.onload = (e: any) => {
        this.uploadedFileList.update(list => [...list, {
          name: file.name,
  ...
  onSubmit() {
    if (this.form.valid) {
      ...
      const payload: Partial<Order> = {
        ...formVal,
        amountVND: formVal.amountVND ? parseInt(formVal.amountVND.replace(/,/g, '')) : 0,
        amountUSD: formVal.amountUSD ? parseInt(formVal.amountUSD.replace(/,/g, '')) : 0,
