      ...
      >
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <select
            ...
            (ngModelChange)="filterProvince.set($event); filterDistrict.set('')"
            class="border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500"
          >
          ...
          <select
            [ngModel]="filterDistrict()"
            (ngModelChange)="filterDistrict.set($event)"
            [disabled]="!filterProvince()"
            class="border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50"
          >
            <option value="">Tất cả Quận/Huyện</option>
            <option *ngFor="let d of districts[filterProvince()]" [value]="d">
              {{ d }}
            </option>
          </select>
        </div>
        ...
        >
          ...
          >
            <h3 class="font-bold text-lg">
              Chi tiết yêu cầu: {{ sel.orderId || sel.id }}
            </h3>
            <button
              (click)="closeAction()"
